...
// Função para medir o tempo de execução com gettimeofday()
double measureTime(int (*searchFunc)(int[], int, int), int arr[], int size, int target) {
    struct timeval start, end;
    gettimeofday(&start, NULL);  // Captura o tempo inicial
    searchFunc(arr, size, target);
    gettimeofday(&end, NULL);    // Captura o tempo final

    // Calcula a diferença de tempo em segundos e microssegundos
